...
def test_no_fork_on_monotonic_progress() -> None:
    ...
    # New ratchet keys reset message numbers and may have different PN values.
...
def test_fork_on_prev_chain_len_change_for_same_ratchet_key() -> None:
    state = ForkDetectionState()
    assert not detect_fork(state, b"rk1", 0, 5)
    # Previous chain length is fixed for one ratchet key.
